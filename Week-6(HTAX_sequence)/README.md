...
1. `short_packet_seq` Generate packets where: `3 <= length <= 10`
2. `long_packet_short_delay_seq` Generate packets where: `40 <= length <= 50` and: `delay < 5`
3. `med_packet_fix_vc_seq` Generate packets where: `10 <= length <= 40` 
...
4. `random_seq` Generate packets using the default HTAX packet constraints without adding additional sequence-level constraints.